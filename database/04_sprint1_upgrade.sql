...
BEGIN
    ...
    CREATE INDEX IX_IntentosLogin_FechaIP ON dbo.IntentosLogin(FechaIntento, DireccionIP);
END;
GO
...
USING (VALUES
 ...
 ('usuarios.desactivar','Activar o desactivar usuarios','Usuarios'),
 ('usuarios.desbloquear','Desbloquear usuarios','Usuarios'),
 ('roles.ver','Ver roles','Roles'),
 ('roles.editar','Administrar permisos de roles','Roles'),
 ('productos.ver','Ver productos','Productos'),
 ('productos.crear','Crear productos','Productos'),
 ('inventario.entrada','Registrar entradas','Inventario'),
 ('inventario.salida','Registrar salidas','Inventario'),
...
BEGIN
    ...
    SELECT @SuperAdmin, p.IdPermiso
    FROM dbo.Permisos p
    WHERE p.Estado=1 AND NOT EXISTS(
        SELECT 1 FROM dbo.RolPermiso rp WHERE rp.IdRol=@SuperAdmin AND rp.IdPermiso=p.IdPermiso
    );
END;
GO
